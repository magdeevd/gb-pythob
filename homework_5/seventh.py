...
def main():
    ...
    with open('files/seventh.txt') as file:
        for line in file:
            name, ownership, earnings, costs = line.split()
            if int(earnings) >= int(costs):
                profits[name] = int(earnings) - int(costs)

    result = [
        profits,
        {"average_profit": sum(profits.values()) / len(profits)}
    ]

    open('result.txt', 'w').write(json.dumps(result))
